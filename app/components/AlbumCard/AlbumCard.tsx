...
export default function AlbumCard({ data, openAlbum }: AlbumCardProps) {
    ...
    return (
        ...
        <div className="AlbumCard m-4 gallery-image relative">
            <Image src={images[0].url} alt={images[0].alt} className="rounded-xl" width="400" height="400" />
            <p className="absolute p-2 m-2 rounded-xl decoration-none bg-[#fcf1e9] top-0 right-0 text-sm">{date}</p>

        </div>
        <button className="absolute p-2 m-2 rounded-xl decoration-none bg-[#fcf1e9] bottom-0 right-0 text-sm z-50" onClick={() => openAlbum(date)}> + </button>
